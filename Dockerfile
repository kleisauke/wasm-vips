...
FROM docker.io/emscripten/emsdk:3.1.33
...
# Emscripten patches
RUN \
  curl -Ls https://github.com/emscripten-core/emscripten/compare/3.1.33...kleisauke:wasm-vips-3.1.33.patch | patch -p1 -d $EMSDK/upstream/emscripten && \
...
RUN \
  curl https://sh.rustup.rs -sSf | CARGO_HOME="/usr/local/cargo" sh -s -- -y \
    ...
    --default-toolchain nightly-2023-03-14 \
    --component rust-src
