...
# Emscripten patches
RUN \
  curl -Ls https://github.com/emscripten-core/emscripten/compare/3.1.40...kleisauke:wasm-vips-3.1.40.patch | patch -p1 -d $EMSDK/upstream/emscripten && \
...
RUN \
  curl https://sh.rustup.rs -sSf | CARGO_HOME="/usr/local/cargo" sh -s -- -y \
    ...
    --default-toolchain nightly-2023-05-24 \
    --component rust-src
