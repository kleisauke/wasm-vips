...
VERSION_GLIB=2.85.4         # https://gitlab.gnome.org/GNOME/glib
...
[ -f "$TARGET/lib/pkgconfig/vips.pc" ] || (
  ...
  meson setup _build --prefix=$TARGET $MESON_ARGS --default-library=static --buildtype=release \
    ...
    -Dheif-module=enabled -Dimagequant=enabled -Djpeg=enabled ${ENABLE_UHDR:+-Duhdr=enabled} \
    ${ENABLE_JXL:+-Djpeg-xl=enabled} -Djpeg-xl-module=enabled -Dlcms=enabled ${ENABLE_SIMD:+-Dhighway=enabled} \
    ${ENABLE_SVG:+-Dresvg=enabled} -Dresvg-module=enabled -Dspng=enabled -Dtiff=enabled -Dwebp=enabled \
    -Dnsgif=true -Dppm=true -Danalyze=true -Dradiance=true -Dzlib=enabled
