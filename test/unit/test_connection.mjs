...
describe('connection', () => {
    ...
    describe('source', () => {
        it('newFromFile', function () {
            const x = vips.Source.newFromFile(Helpers.JPEG_FILE);

            expect(x.filename).to.equal(Helpers.JPEG_FILE);
        });
        it('newFromMemory', function () {
            const data = colour.writeToBuffer('.jpg');
            const x = vips.Source.newFromMemory(data);

            expect(x.filename).to.be.empty;
        });
    });
    ...
    describe('image', () => {
        describe('newFromSource', () => {
            ...
            it('custom', function () {
                const stream = vips.FS.open(Helpers.JPEG_FILE, 'r');
                ...
                const image2 = vips.Image.newFromFile(Helpers.JPEG_FILE, {
                    access: 'sequential'
                });
        ...
        describe('writeToTarget', () => {
            ...
            it('custom', function () {
                ...
                let image = vips.Image.newFromFile(Helpers.JPEG_FILE, {
                    access: 'sequential'
                });
                image.writeToTarget(target, '.png');

                image = vips.Image.newFromFile(Helpers.JPEG_FILE, {
                    access: 'sequential'
                });
