...
describe('create', () => {
    ...
    it('buildlut', function () {
        ...
        p = lut.getpoint(0, 0);
        Helpers.assert_almost_equal_objects(p, [0.0, 100.0]);

        p = lut.getpoint(64, 0);
        Helpers.assert_almost_equal_objects(p, [5.0, 95.0]);
    });
    ...
    it('invertlut', function () {
        let lut = vips.Image.newFromArray([
            [0.1, 0.2, 0.3, 0.1],
            [0.2, 0.4, 0.4, 0.2],
            [0.7, 0.5, 0.6, 0.3]
        ]);
        ...
        let p = im.getpoint(0, 0);
        Helpers.assert_almost_equal_objects(p, [0, 0, 0]);
        p = im.getpoint(255, 0);
        Helpers.assert_almost_equal_objects(p, [1, 1, 1]);
        p = im.getpoint(.2 * 255, 0);
        expect(p[0]).to.be.closeTo(0.1, 0.1);
        p = im.getpoint(.3 * 255, 0);
        expect(p[1]).to.be.closeTo(0.1, 0.1);
        p = im.getpoint(.1 * 255, 0);
        expect(p[2]).to.be.closeTo(0.1, 0.1);
    });

    it('matrixinvert', function () {
        // 4x4 matrix to check if PLU decomposition works
        let mat = vips.Image.newFromArray([
            [4, 0, 0, 0],
            [0, 0, 2, 0],
            [0, 1, 2, 0],
            [1, 0, 0, 1]
    ...
    it('xyz', function () {
        ...
        Helpers.assert_almost_equal_objects(p, [45, 35]);
    });

    it('zone', function () {
        let im = vips.Image.zone(128, 128);
        expect(im.width).to.equal(128);
        expect(im.height).to.equal(128);
        expect(im.bands).to.equal(1);
        expect(im.format).to.equal('float');
